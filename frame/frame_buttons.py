...
def frame_buttons(column, row, sticky):
    ...
    process_button = tk.Button(frame_buttons, text="Рассчитать", state="disabled", command=lambda: choose_calculate_type(True, False))
    process_button.place(relx=0.86, rely=0.1)
    buttons["process_button"] = process_button
    plots_button = tk.Button(frame_buttons, text="Графики", state="disabled", command=lambda: choose_calculate_type(False, True))
