...
def open_xls_inputs(_):
    if(values["value_of_checkBox_inputsource"].get() == 1):
        for widget in frames["_1subframe2"].winfo_children():
            ...
            values["filename_value"] = filename_value
            file_inp_xls = tk.Entry(frames["_1subframe2"], textvariable=filename_value)
            ...
            add_button = tk.Button(frames["_1subframe2"], text="Добавить", state="disabled", command=lambda:2+2)
